...
// 图案函数
// ─────────────────────────────────────────────────────────────
export function patternCircleGrid(p) {
    const cells = 9;
    const cellSize = 80;
...
export function patternDiagonal(p) {
    let s = '';
    const step = 24;
...
export function patternConcentric(p) {
    let s = '';
    const cx = W / 2;
    const cy = H / 2;
...
export function patternTriangles(p) {
    let s = '';
    const size = 70;
...
// SVG 生成
// ─────────────────────────────────────────────────────────────
export function genSvg(seed, paletteName) {
    const h = hash(seed);
...
export async function readPost(filename) {
 ...
 */
export async function injectCoverField(files, { dryRun }) {
    let injected = 0;
    let skipped = 0;
...
async function main() {
    ...
    if (mode === 'files') {
        ...
        if (inject)
            await injectCoverField(files, { dryRun: false });
        return;
    }
    // mode === 'all'
    const all = await listPosts();
