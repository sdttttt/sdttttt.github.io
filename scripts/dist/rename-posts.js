 ...
 */
export function computeHash6(body) {
 ...
 */
export function rewriteCoverImage(raw, oldSlug, newSlug) {
 ...
 */
export async function computeCoverUpdate(meta, oldSlug, newSlug) {
    ...
    if (image !== expected)
        return null; // 用户自定义了别的封面路径，不动
    const oldCoverPath = join(COVERS_DIR, `${oldSlug}.svg`);
    if (!(await exists(oldCoverPath)))
        return null;
    return {
        oldCoverPath,
...
export function detectCollisions(plans) {
    const map = new Map();
...
function printPlan(p, prefix) {
    ...
    if (p.cover) {
        console.log(`    封面: ${basename(p.cover.oldCoverPath)} → ${basename(p.cover.newCoverPath)}`);
    }
}
async function main() {
