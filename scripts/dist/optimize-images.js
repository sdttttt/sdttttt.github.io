...
function ratio(original, optimized) {
...
function applyResize(pipeline, width) {
    if (!width)
        return pipeline;
...
function applyFormat(pipeline, ext, quality) {
    switch (ext) {
        case '.png':
...
async function optimizeFile(filePath, opts) {
    ...
    if (!opts.variantsOnly) {
        ...
        if (opts.dryRun) {
            console.log(`[dry-run] ${filePath}: ${formatBytes(originalSize)} → ${formatBytes(optimizedBuf.length)} ${ratio(originalSize, optimizedBuf.length)}${willWrite ? '' : ' [跳过]'}`);
        }
        else if (willWrite) {
            await writeFile(filePath, optimizedBuf);
            console.log(`✓ ${filePath}: ${formatBytes(originalSize)} → ${formatBytes(optimizedBuf.length)} ${ratio(originalSize, optimizedBuf.length)}`);
        }
        else {
            console.log(`⏭ ${filePath}: ${formatBytes(originalSize)} → ${formatBytes(optimizedBuf.length)} ${ratio(originalSize, optimizedBuf.length)}，跳过`);
        }
    }
    // 2) 生成 .webp 变体
    if (opts.webp && ext !== '.webp') {
        const webpPath = filePath.replace(/\.[^.]+$/, '.webp');
        const webpBuf = await applyResize(sharp(originalBuf).rotate(), opts.width).webp({ quality: opts.quality }).toBuffer();
        if (opts.dryRun) {
...
async function main() {
    ...
    console.log(`找到 ${targets.length} 张图片：quality=${opts.quality}${opts.width ? `, maxWidth=${opts.width}` : ''}${opts.webp ? ', 生成 .webp 变体' : ''}${opts.variantsOnly ? ' [仅变体，不覆盖原图]' : ''}${opts.dryRun ? ' (dry-run)' : ''}\n`);
