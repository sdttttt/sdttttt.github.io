...
import { extractLinksOutsideCodeBlocks, shouldSkip, checkUrl, checkUrlGet, walkMarkdown, } from '../check-dead-links.js';
...
describe('checkUrl', () => {
    test('200 返回 ok', async () => {
        globalThis.fetch = mock.fn(() => Promise.resolve(new Response(null, { status: 200, statusText: 'OK' })));
    ...
    test('404 返回失败', async () => {
        globalThis.fetch = mock.fn(() => Promise.resolve(new Response(null, { status: 404 })));
        const result = await checkUrl('https://example.com');
...
describe('walkMarkdown', () => {
    test('递归遍历 Markdown 文件', () => inTempDir(async () => {
