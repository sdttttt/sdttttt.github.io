...
_(查询的列被索引覆盖，这种情况是不会发生回表的，只是进行索引扫描罢了)_

**NULL**

_（既没有`Using index`，也没有`Using where Using index`，也没有`using where`）_
...
_（`using where` 意味着通过索引或者表扫描的方式进程`where`条件的过滤，
反过来说，也就是没有可用的索引查找，这里的type都是all，说明MySQL认为全表扫描是一种比较低的代价。）_
