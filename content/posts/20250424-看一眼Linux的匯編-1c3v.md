...
title: "看一眼Linux的匯編"
date: 2025-04-24
description: "以同一段加法程序在 Linux 平臺編譯後的彙編為例，對比 Windows 結果，分析參數傳遞、棧幀、計算、返回值與退出過程。"
tags: ["學習", "思考"]
author: sdttttt
draft: false
cover:
  image: "images/covers/20250424-看一眼Linux的匯編-1c3v.svg"
  alt: ""
  hidden: false
aliases: ["/posts/2025042401vukf/"]
language: "zh-tw"
---

繼續上一篇文章，還是那個代碼，這次我們在Linux上編譯試試，看看和windows有什麼不同。
...
```nasm
   ...
   ;後面把參數放進去，調用add，然後吧eax裡的返回值取出到rbp-4這個地址裡，清零
   ;leave的看起來像是linux平臺下獨有的指令
   ;這個指令其實基本等同於 mov rsp, rbp; pop rbp
   ;實際上就是回復上下文
   ...
   ;這裡也很簡單，前兩句還是保存上下文，不過這次沒有開闢棧幀
   ;直接把兩個寄存器的值移動到內存地址裡再放在寄存器裡再計算？
